...
public class UserMethods {
	...
	{
		if(userLogin.userIdValidation(userId) == false)
		{
	...
	public double checkBalance(long accountNo) throws SQLException, UserDefinedException 
	{
		...
		accountMap = viewAccount(accountNo);
		Account accountPojo = accountMap.get(accountNo);
		
		if (accountPojo == null) 
		{
			throw new UserDefinedException("No Account Available");
		}
	...
	public  Map<Long, Account> viewAccount(long accountNo) throws SQLException, UserDefinedException 
	{
		 Map<Long, Account> accountMap = tableAccessObj.getAccountInfo(accountNo);
